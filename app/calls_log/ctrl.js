...
(function(app){
    (function(CallsLogItem){
        ...
        angular.module("IVRY-App").controller("CallLogCtrl",["$scope","$log","linesFilter","$uibModal",function($scope,$log,linesFilter,$uibModal){
            var _this=this;
            $scope.isOn=false;
            $scope.$watch(function(){return _this.editObj},function(nVal){
                if(nVal!=null){
                $scope.isOn=false;
                $scope.isOn=true;
                }
            ...
            $scope.currentPage = 1;
            $scope.numPerPage=10;
            $scope.advancedFilter=[{attribute:null,operator:null,value:null},];
            ...
            this.fromToObj={from:new Date(),to:new Date()};
            $scope.$watch(function(){return _this.fromToObj.from;},function(nVal){
            ...
            $scope.$watch(function(){return _this.editObj;},function(nVal){
                if (nVal!=undefined || nVal!=null){
                    try{_this.wavesurfer.destroy();}catch(ex){}
...
//                    _this.wavesurfer.load("/assets/libs/wavesurfer.js-master/example/split-channels/stereo.mp3");
            ...
            for(var i=0,idx=1;i<3;i++)
            for(var j=0;j<3;j++)
            for(var k=0;k<3;k++){
                this.callsLog.push(CallsLogItem("New Case Name"+(i+1),"New Identity Name"+(((i+1)*(j+1))),"New Line Name"+idx,1200+i+j+k,new Date("5/28/2016 12:09"),new Date("5/28/2016 12:41"),23,13,false,"","/assets/libs/wavesurfer.js-master/example/split-channels/stereo.mp3"));
                idx++;
            }
            $scope.linesTreeObj=[
                {title:'Case Number 1',checked:true,
                childs:[
                    {title:'Identity Number 11',checked:true,
                    childs:[
                        {title:"Line Number 111",checked:true,id:1206},
                        {title:"Line Number 112",checked:true,id:1205},
                        {title:"Line Number 113",checked:true,id:1204}
                    ]},
                    {title:'Identity Number 12',checked:true,
                    childs:[
                        {title:"Line Number 121",checked:true,id:1201},
                        {title:"Line Number 122",checked:true,id:1202},
                        {title:"Line Number 123",checked:true,id:1203}
                    ]}
                ]},
                {title:'Case Number 2',checked:false,
                childs:[
                    {title:'Identity Number 21',checked:false,
                    childs:[
                        {title:"Line Number 211",checked:false,id:1207},
                        {title:"Line Number 212",checked:false,id:1208},
                        {title:"Line Number 213",checked:false,id:1209}
                    ]}
                ]},
                {title:"Case 3",checked:false,},
                {title:"Case 4",checked:false,
                childs:[
                    {title:"Identity 41",checked:false,}
                ]},
                {title:"Case 5",checked:false,
                childs:[
                    {title:"Identity 51",checked:false,
                    childs:[
                        {title:"Line 511",checked:false,id:1210}
                    ]}
                ]}
            ];
            $scope.columns={childs:[
                {title:'Case Name',prop:"caseName",isOn:true},
                {title:'Identity Name',prop:"identityName",isOn:true},
                {title:'Line Name',prop:"lineName",isOn:true},
                {title:'Line ID',prop:"lineId",isOn:true},
                {title:'Date',prop:"startDate",isOn:true,type:"date"},
                {title:'Start Time',prop:"startDate",isOn:false,type:"time"},
                {title:'End Time',prop:"endDate",isOn:false,type:"time"},
                {title:'Duration',prop:"duration",isOn:false,type:"time"},
                {title:'Comment',prop:"comment",isOn:false},
                {title:'Calling Number (a_number)',prop:"sipCallingParty",isOn:false},
                {title:'Called Number (b_number)',prop:"sipCalledParty",isOn:false},
            ]};
